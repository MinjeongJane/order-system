...
class OrderControllerIntegrationTest @Autowired constructor(
    ...
    fun tearDown() {
        ...
        // 크레딧 충전 내역 삭제
        jdbcTemplate.update("DELETE FROM USER_CREDIT WHERE id = ?", testUserId)
    ...
    @Test
    fun `크레딧 충전시 성공 메시지 반환`() {
        val request = CreditChargeRequest(userId = testUserId, credits = 100)
        ...
        assert(json["value"].asText() == "크레딧 충전이 완료되었습니다.")
    }
